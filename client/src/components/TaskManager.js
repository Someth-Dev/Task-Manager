...
const TaskManager = ({ language = 'km', darkMode = false }) => {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost/task-manager/api/categories.php');
  ...
  const fetchTasks = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost/task-manager/api/index.php?page=${page}&search=${search}`);
  ...
  const fetchReminders = async (taskId) => {
    try {
      const response = await axios.get(`http://localhost/task-manager/api/reminders.php?task_id=${taskId}`);
  ...
  const fetchShares = async (taskId) => {
    try {
      const response = await axios.get(`http://localhost/task-manager/api/shares.php?task_id=${taskId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        await axios.put('http://localhost/task-manager/api/index.php', { id: editId, ...task });
        showNotification(`${t.taskTitle} ${title} ${language === 'km' ? 'បានធ្វើបច្ចុប្បន្នភាព' : 'updated successfully'}`, 'success');
        setEditId(null);
      } else {
        await axios.post('http://localhost/task-manager/api/index.php', task);
  ...
  const handleAddReminder = async (taskId) => {
    ...
    try {
      await axios.post('http://localhost/task-manager/api/reminders.php', { task_id: taskId, reminder_date: newReminder });
      setNewReminder('');
  ...
  const handleShareTask = async (taskId) => {
    ...
    try {
      await axios.post('http://localhost/task-manager/api/shares.php', { task_id: taskId, email: shareEmail });
      setShareEmail('');
      fetchShares(taskId);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete('http://localhost/task-manager/api/index.php', { data: { id } });
      fetchTasks();
